...
function [fuhuan,flagJ]=greedyhuan_jiacan_buchongfu(J,GF,INF,alph,buchongfu)
[G,N]=size(GF);
flagJ=0;
...
for iii=1:ceil((G+N)*alph)
    
    [L,X]=min(min(Jbeiyong'));
    [L,Y]=min(min(Jbeiyong));
    Jbeiyongbeiyong=Jbeiyong;
    for ysysy=1:N
        if buchongfu(Y)==1
        Jbeiyongbeiyong(:,Y)=INF;
        [L,X]=min(min(Jbeiyongbeiyong'));
        [L,Y]=min(min(Jbeiyongbeiyong));
        else 
            break
        end
    end
